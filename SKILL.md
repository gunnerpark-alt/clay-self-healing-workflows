...
- **Audience records and activities.** Those are data, not config — the same way workflow *runs* aren't versioned, only the workflow *definition* is. Don't put customer/lead data in git.
...
3. **Copy the contents of `templates/` into the target repo:**
...
4. **Ask which workflows to track** (name or ID — `clay workflows list` if they don't know). Run:
   ```
